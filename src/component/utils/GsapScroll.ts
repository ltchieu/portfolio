...
export function setCharTimeline(
  ...
  // Initialize position to center via GSAP to prevent CSS transform conflicts
  ...
  if (window.innerWidth > 1024) {
    ...
    if (tl2 && neckBone && monitor && screenLight) {
      tl2
        .to(
          camera.position,
        ...
        .fromTo(
          ...
          { pointerEvents: "none", xPercent: -62, delay: 2, duration: 5, immediateRender: false },
          0
        )
      ...
      const whatBoxEl = document.querySelector(".what-box-in");
      if (whatBoxEl) {
        tl2.fromTo(
          ".what-box-in",
          { display: "none" },
    ...
    const whatBoxInEl = document.querySelector(".what-box-in");
    if (whatBoxInEl) {
  ...
  // Cleanup interval on page unmount (handled by Scene.tsx or globally)
